...
export class ExtensionSlotRegistry {
    ...
    mount(name, element) {
        ...
        if (!isNode) throw new Error(`Extension slot ${name} requires a DOM element.`);
    ...
    async mountAll(extensions, context) {
        ...
        for (const extension of Array.isArray(extensions) ? extensions : []) {
            ...
            if (extension?.apiVersion !== EXTENSION_API_VERSION || typeof extension?.mount !== 'function') {
                this.onError(`[Extensions] Skipped incompatible extension ${id}. Expected API version ${EXTENSION_API_VERSION}.`);
                continue;
            }
            tasks.push(this.mountOne(extension, id, context));
        }
        await Promise.all(tasks);
        this.slots.refreshAll((name, error) => this.onError(`[Extensions] Failed to refresh slot ${name}.`, error));
    }
