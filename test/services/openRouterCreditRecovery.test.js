...
test('cancellation interrupts settlement wait and prevents another request', async () => {
    ...
    let calls = 0;
    const response = rejection('Busy', 'openrouter_in_flight_budget', { 'Retry-After': '30' });
    ...
    assert.equal(calls, 1);
    assert.equal(response.bodyUsed, true, 'discarded rejection is canceled before the settlement wait');
