...
test('account billing uses the SuperTokens transport without renderer bearer tokens', async () => {
    const calls = [];
    const authProvider = new BillingAuthProvider({
        accountService: {
            getState() {
                return { accountId: 'OA-ACCOUNT', sessionVerified: true };
            }
        },
        storage: memoryStorage(),
        orgBase: 'https://org.example.test'
    });
    const client = new BillingClient({
        authProvider,
        ...
        fetchImpl: async () => {
            throw new Error('Account billing must not use anonymous fetch');
        },
        accountFetchImpl: async (url, options) => {
            calls.push({ url, options });
    ...
    assert.equal(calls.length, 1);
    assert.ok(calls[0].url.endsWith('/api/billing/status'));
