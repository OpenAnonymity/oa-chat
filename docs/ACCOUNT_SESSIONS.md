...
- `encryptedSyncService` keeps only non-extractable HKDF/HMAC derivation keys.
  All protected sync requests use `sessionService.fetch`; its outer retry
  handles transient network/server failures, not authentication refresh.
