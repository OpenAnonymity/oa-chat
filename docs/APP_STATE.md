...
- 2026-04-28: Right-panel Memory/System-Prompt sections (Rory #8).
  - Right panel (`RightPanel.js`) now has two new sections inserted between the existing top section and the Activity Timeline: **Memory** (flex-basis 18%, min 90px) and **System Prompt** (flex-basis 12%, min 70px). Both have internal scroll. Memory section uses the shared `services/memoryInstances.js` instance — `memoryFileSystem.exportAll()` is the same source as the Memory Editor sidebar. System Prompt section reads `app.state.systemPromptPresets`. Both header buttons use the same pencil edit icon for visual consistency.
  - **Reverted on 2026-04-28**: per-fact `×` inside the retrieval bubble's `[[user_data]]` highlights (Rory #5/#9) and the per-file `×` on right-panel highlighted file rows. User found both too noisy. The bubble back to plain `renderTaggedPromptInline(text)` rendering; the panel's used-file rows back to the green dot only. The full-prompt modal editor (Cmd-click "View prompt") still works for prompt edits and writes to `ciPromptDraft.editedFullPrompt`. Note: the inline bubble preview reads `fullPrompt || editedFullPrompt` (in that order), so modal-edited prompts may not reflect in the bubble until re-render — pre-existing behavior, not in scope to fix.
  - Highlighting is **most-recent-retrieval-scoped, not cumulative**: Memory rows are highlighted only for the files referenced by the *latest* message in the current session that has `ciPromptDraft.memoryFiles`. Sending a new turn that triggers a different retrieval drops the previous highlights. (If the user sends a turn without memory mode, the previous highlights persist until another retrieval happens, since `computeUsedMemoryFiles` walks for the *last seen* memoryFiles entry.) System Prompt rows are highlighted for `currentSession.systemPromptId` (or "Default" when null).
  - Click on a memory file → `memoryEditor.openToFile(path)`. Click on a preset row → `app.openSystemPromptModal()` followed by `app.selectSystemPromptForEdit(id)` on the next animation frame (the modal must be in DOM before selecting).
  - Refresh hooks for the right panel are scattered intentionally — keep them aligned when changing memory flow:
    - `RightPanel.onSessionChange()` — reloads both sections (used files + active preset are session-scoped)
    - `runMemoryAugmentFlow()` after writing `ciPromptDraft` — refreshes Memory used-files highlight
    - `runPostTurnMemoryExtraction()` on success — refreshes Memory tree (`reloadFiles: true`)
    - `setSessionSystemPrompt()`, `saveSystemPromptPreset()`, `deleteSystemPromptPreset()` — refreshes System Prompt section
    - `MemoryEditor._loadFileTree()` — refreshes Memory tree (`reloadFiles: true`) so create/save/delete in the editor reflects in the panel without a session switch
  - Memory tree folder expansion is **derived from used-files only when the used-files set changes**. `computeUsedMemoryFiles()` snapshots the previous and new used-files signatures (sorted-joined paths) and only resets `RightPanel.memoryTreeExpandedDirs` when they differ. This means: session switches and new retrievals reset folder expansion to "exactly the directories that contain a highlighted file"; plain re-renders (the user's own click on a folder, post-extraction tree refreshes that don't change the highlighted set) preserve the user's manual expand/collapse. The first version of this code reset on every compute, which made manual folder clicks appear non-responsive — the click toggled the set, then `computeUsedMemoryFiles` immediately re-derived from the (unchanged) used-files, overwriting the toggle.
  - **Deferred (#8 follow-up)**: per-session "files written during extraction" diffing needs a `nanomem` ingest-result change to expose write paths. Currently `runPostTurnMemoryExtraction` returns `{status, writeCalls}` only.
- 2026-04-28: Help is split into two modals — `AboutModal` (intro) and `ShortcutsModal` (customizable shortcuts) — backed by a new `shortcutManager` service.
  - **Toolbar buttons.** `#about-btn` (info-circle icon) and `#shortcuts-btn` (terminal-prompt icon) sit in that order in the chat-area right toolbar, both before `#share-btn`. Tooltip on Shortcuts shows the current binding string ("Keyboard shortcuts (?)" by default).
  - **Single source of truth for shortcuts.** `chat/services/shortcutManager.js` owns defaults + persisted overrides (`localStorage` key `oa-shortcuts`). The `mod` modifier maps to Cmd on macOS and Ctrl elsewhere — store one binding, works on both. When adding a new customizable action: register it in `DEFAULT_SHORTCUTS`, `ACTION_LABELS`, and `ACTION_ORDER`, then dispatch via the `matchAction(e, 'id')` helper in `app.js`. ShortcutsModal picks it up automatically.
  - **Dispatch in `app.js`.** The global keyboard handler routes through `shortcutManager.matches(event, actionId)`. There's a `shortcutCanFireWhileTyping(actionId)` helper that returns true only for bindings with a `mod` or `alt` modifier — no-mod bindings (like the default `?`) bail out when the active element is an input/textarea/contentEditable so they don't fire mid-message. `Cmd+Z` (file-paste undo) is intentionally not customizable — it's the platform's standard undo combo and we don't want to fight that convention.
  - **Capture phase is load-bearing.** `ShortcutsModal._startEditing` adds the capture handler with `addEventListener('keydown', handler, true)` (capture phase). Without that, a user trying to rebind `⌘K` to something else would briefly trigger the model picker between mousedown-on-Edit and the actual rebinding press — the bubble-phase global handler in `app.js` would still see the old binding first. The capture handler always `preventDefault` + `stopPropagation` once it accepts a non-modifier key.
  - **Conflict policy.** A new binding that collides with another action is rejected with an inline error; the user must pick a different combo or reset the conflicting action. We never silently clear someone else's binding — it leads to "I rebound X and now Y mysteriously stopped working" surprises.
  - **Don't unify the modals.** Keep about content in `AboutModal` and shortcut content in `ShortcutsModal`. The previous combined HelpModal was deliberately split because users wanted shortcuts customizable without scrolling past 4 sections of intro copy.
  - **Contextual shortcuts stay contextual.** The read-only `CONTEXTUAL_SHORTCUTS` table in `ShortcutsModal.js` documents Enter-to-send, ⌘+Enter-to-confirm-edit, ⌘S in memory editor, model-picker arrow nav, etc. These are intentionally not customizable because they're tightly coupled to widget behavior. Update this table when adding/removing such a shortcut.
- 2026-04-28: Memory Editor create flow uses surgical DOM updates and a focused-folder concept.
  - `MemoryEditor` (in `chat/components/MemoryEditor.js`) tracks `this.focusedDir` — the folder the user is "in". Set on expand or file-select; cleared on collapse-of-focused / root-file-select / blank-sidebar-click. Persisted across renames (tracks to the new name) and cleared if the focused folder is deleted.
  - The focused folder gets an inset 3px accent stripe on its left edge, applied both via static render (in `_renderSidebar` as an inline `box-shadow` style on the dir-row) and surgically by `_setFocusedDir` between renders. Both paths must stay in sync — change the inline style in one place and the surgical helper too.
  - Clicking *New File* / *New Folder* and clicking elsewhere to cancel are now surgical: no full `render()` is called. `_startNewFile`/`_startNewFolder` invoke `_injectCreateInputDom(kind)` which finds the right container (focused folder's `[data-dir-children]` for files, sidebar root otherwise) and appends a wrapper built by `_renderInlineCreateInput`. `_removeInlineCreateInput` removes the wrapper and resets state. The full `render()` path was causing visible flicker on every click.
  - `_renderInlineCreateInput(kind, indent)` is the shared markup builder used by *both* the static render in `_renderSidebar` and the surgical inject. Keep them aligned — same markup, same `data-role="inline-create-input"` attribute (used by the sidebar click handler to skip the focus-clear branch), same input IDs.
  - `_renderSidebar` includes a *phantom dir* for `focusedDir` when it has no files yet (the brief window after typing a new folder name + Enter, before the first file lands). This gives the new-file input a children container to nest into so the user sees their just-named folder with the input nested inside.
  - New folders are always top-level. The memory FS is flat one-level by storage design — `_getDirectoryStructure` only splits at the first `/`, so genuinely nested folders aren't supported by the sidebar rendering. The new-folder input always lives at root regardless of focus; after Enter, focus moves to the new folder and the file-step input lands inside it via the phantom-dir path. Typing `subdir/foo` in a focused-folder file input still writes `personal/subdir/foo.md` and renders as a file named `subdir/foo.md` inside `personal` — that's storage working correctly with flat rendering, not a bug to chase.
  - `_wireCreateInput(input, kind)` is the unified handler attachment used by both surgical inject and the post-render `_attachInputListeners`. It handles Enter (create), Escape (surgical cancel), and deferred-blur cancel. The deferred-blur uses `setTimeout(0)` + `inputEl.isConnected` so a click on a sibling button (e.g. *New Folder* while the *New File* input is focused) reaches its own click handler before the wrapper is torn down — a synchronous cancel-on-blur would detach the button between mousedown and click and lose the second click. **Do not "simplify" by removing the `setTimeout` — it is load-bearing.**
  - `_attachRenameListener` for folder renaming intentionally has a *different* blur policy (auto-commit on changed value). Do not unify with the create-flow cancel-on-blur — rename is editing an existing thing, create is provisional.
