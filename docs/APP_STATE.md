...
- 2026-04-10: Root `oa-chat` now has a browser-only memory mode wired through the `nanomem` submodule.
  ...
  - There is still no automatic per-turn memory writing/extraction in the chat loop. The write-side agent only runs through explicit memory actions such as `Backfill`, `Import`, or direct memory editing, not after every send.
  ...
  - Root `oa-chat` now mirrors `memory-chat`'s post-response extraction pattern when memory mode is on. After each successful assistant response, the app kicks off a non-blocking background `nanomem.ingest(...)` run for the current session.
